...
export class GraphifyMcpClient {
    ...
    async establishConnection() {
        ...
        try {
            ...
            transport.onclose = () => {
                if (this.generation !== currentGeneration || this.isDisposed || !handshakeComplete)
                    return;
     ...
     */
    async reconnectAttempt(triggerGeneration) {
        if (this.isDisposed || this.generation !== triggerGeneration)
            return;
        try {
            ...
            if (this.isDisposed || this.generation !== triggerGeneration + 1)
                return;
    ...
    scheduleReconnect(currentGeneration) {
        ...
        if (this.reconnectAttempts >= this.reconnectConfig.maxAttempts) {
            this.setState('error');
            this.logger?.error(`[dsh-graphify] Reconnect failed after ${this.reconnectAttempts} consecutive attempts. Will not retry automatically until next invocation.`);
            return;
        }
        this.reconnectAttempts++;
        const delay = Math.min(this.reconnectConfig.initialDelayMs * Math.pow(2, this.reconnectAttempts - 1), this.reconnectConfig.maxDelayMs);
        this.setState('reconnecting');
        this.logger?.warn(`[dsh-graphify] Scheduling reconnect in ${delay}ms (attempt ${this.reconnectAttempts}/${this.reconnectConfig.maxAttempts})...`);
    ...
    async listTools() {
        await this.init();
        if (!this.client)
            throw new Error('Graphify MCP client is not connected');
     ...
     */
    async readResource(uri, signal, timeoutMs) {
        if (signal?.aborted)
            throw new Error('Resource read aborted by signal');
        await this.init();
        if (!this.client)
            throw new Error('Graphify MCP client is not connected');
        const effectiveTimeout = timeoutMs ?? this.defaultTimeoutMs;
        try {
            ...
            if (signal?.aborted)
                throw new Error(`Resource read ${uri} cancelled`);
            throw err;
        }
    }
    /**
