...
import { checkGraphFreshness, ProjectUpdateCoalescer, evaluateAutoUpdateEligibility, performPostUpdateValidation, writeGraphifyIndexMetadata, } from "./freshness.js";
import { collectGraphifyStatus, formatGraphifyStatus } from "./status.js";
 ...
 */
export function getPrefixedToolName(baseName, prefix) {
    if (!prefix)
        return baseName;
...
export function isPathContained(parentDir, childPath) {
    try {
 ...
 */
export function validateProjectPathAccess(explicitPath, execution, config) {
    if (!explicitPath || config.allowExternalProjects) {
        return;
    }
    const sessionWorkspace = execution?.agent?.session.header.cwd ||
        config.cwd ||
        process.cwd();
    if (!sessionWorkspace) {
        return;
    ...
    if (!isPathContained(sessionWorkspace, explicitPath)) {
        throw new Error(`Access to external project path "${explicitPath}" is blocked. Model-controlled tool calls are restricted to the active session workspace ("${sessionWorkspace}"). Set \`allowExternalProjects: true\` in cordis.yml to allow model access outside the workspace.`);
    }
}
/**
 * Creates Graphify's native tools, doctor tool, plus capability and resource accessors.
 */
export function createGraphifyToolDefinitions(client, config, detectedGraph, customResolver) {
    ...
    async function executeTool(rawName, rawArgs, execution, isQueryTool = false) {
        try {
            const args = { ...rawArgs };
    ...
    const allDefinitions = [
        // 1. query_graph
        {
            ...
            execute: (args, exec) => executeTool('query_graph', args || {}, exec, true),
        },
        // 2. get_node
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('get_node', args || {}, exec, true),
        },
        // 3. get_neighbors
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('get_neighbors', args || {}, exec, true),
        },
        // 4. shortest_path
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('shortest_path', args || {}, exec, true),
        },
            ...
            execute: async (_args, exec) => {
                ...
                if (explicitPath) {
                    try {
                        ...
                        return toLosslessJson({
                            text: `Error executing graphify_status: ${err.message}`,
                            overall: 'error',
                            projectRoot: explicitPath,
                            graphExists: false,
                            freshness: { state: 'unknown', reason: err.message },
        ...
        // 6. get_community
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('get_community', args || {}, exec, true),
        },
        // 7. god_nodes
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('god_nodes', args || {}, exec, true),
        },
        // 8. graph_stats
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('graph_stats', args || {}, exec, true),
        },
        // 9. list_prs
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('list_prs', args || {}, exec),
        },
        // 10. get_pr_impact
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('get_pr_impact', args || {}, exec),
        },
        // 11. triage_prs
        {
            ...
            output: { schema: COMMON_OUTPUT_SCHEMA, render: renderOutput },
            timeoutMs: config.timeoutMs,
            execute: (args, exec) => executeTool('triage_prs', args || {}, exec),
        },
 ...
 */
export function registerGraphifyTools(ctx, client, config, detectedGraph, resolver) {
